...
unsafe fn check_physical_device(
    ...
    if features.sampler_anisotropy != vk::TRUE {
        return Err(anyhow!(SuitabilityErrror("No sampler anisotropy.")));
    }
...
unsafe fn create_swapchain(
    ...
    let extent = get_swapchain_extension(window, support.capabilities);
...
unsafe fn create_texture_image(
    ...
    let (info, mut render) = decoder.read_info()?;
    let mut pixels = vec![0; info.buffer_size()];
    render.next_frame(&mut pixels)?;
...
fn load_model(data: &mut AppData) -> Result<()> {
    let mut render = BufReader::new(File::open("resources/viking_room.obj")?);
    let (models, _) = tobj::load_obj_buf(&mut render, true, |_| {
        ...
        for index in &model.mesh.indices {
            ...
            let vertex = Vertex {
                ...
                tex_coord: glm::vec2(
                    ...
                    1. - model.mesh.texcoords[tex_coord_offset + 1],
                ),
            };
    ...
    unsafe fn get(
        ...
        for (index, properties) in properties.iter().enumerate() {
            if instance.get_physical_device_surface_support_khr(
                ...
                present_option = Some(index as u32);
                break;
            }
        }

        if let (Some(graphics), Some(present)) = (graphics_option, present_option) {
